    ...
    <form @submit.prevent="handleSubmit">
      <div v-for="(field, index) in fields" :key="index" class="form-field">
        ...
        <textarea
          ...
          :placeholder="field.placeholder"
        ></textarea>
        <div v-else-if="field.type === 'select' && field.name === 'participants'" class="participants-field">
          <label>{{ field.label }}</label>
          <div class="selected-participants">
            <div v-for="(participant, index) in formData[field.name]" :key="index" class="selected-participant">
              {{ participant }}
...
export default {
  ...
  data() {
    ...
    this.fields.forEach(field => {
      formData[field.name] = field.type === 'checkbox' ? false : (field.multiple ? [] : '');
    });
    return {
      formData,
      newParticipant: ''
  ...
  methods: {
    ...
    addParticipant() {
      if (this.newParticipant.trim() && !this.formData.participants.includes(this.newParticipant.trim())) {
